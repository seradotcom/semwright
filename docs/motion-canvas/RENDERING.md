...
7. The helper starts the pinned full Chromium executable in new-headless mode with `--remote-debugging-address=127.0.0.1 --remote-debugging-port=0`, reads the bounded `DevToolsActivePort` file from its disposable profile, and attaches Playwright with `connectOverCDP`. Bubblewrap's `network=false` namespace contains only loopback, so this unauthenticated CDP endpoint is sandbox-internal and not host/LAN reachable. The generated project still uses request interception at `semwright.invalid`; all external page requests are aborted and no Vite HTTP server exists.
