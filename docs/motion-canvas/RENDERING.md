...
The manual `Motion Canvas launch film` workflow renders the complete 1,560-frame / 52-second 1920×1080 project on an ephemeral GitHub runner. A fixed FFmpeg argument vector converts the validated PNG sequence to a mezzanine because the current MLT driver imports bounded media files rather than image sequences. The real MLT provider then assembles video + deterministic WAV and performs the final H.264/AAC encode. Every curated MLT profile remains on the certified conservative offline/no-drop processing path (`real_time=-1`, `threads=2`). CI demonstrated that MLT worker parallelism can terminate with SIGSEGV both on lossless rendering and on the full H.264 launch-film path, so the driver does not trade stability for frame-worker throughput. H.264 keeps libx264 CRF 18 with the `veryfast` preset; this confines the speed/size tradeoff to the encoder while preserving the same semantic timeline, resolution, audio path and MLT provider.
