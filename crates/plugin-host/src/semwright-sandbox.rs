...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        match args.next().as_deref() {
            Some("--write-root") => {
                let p = args.next().ok_or("write root missing")?;
                if !p.starts_with("/workspace/") || p.contains("..") || p.contains('\0') {
    ...
    for (resource, limit) in [
        (libc::RLIMIT_NOFILE, 128u64),
        (libc::RLIMIT_NPROC, 32),
        (libc::RLIMIT_CPU, 20),
        (libc::RLIMIT_AS, 536_870_912),
        (libc::RLIMIT_FSIZE, 16_777_216),
        (libc::RLIMIT_CORE, 0),
    ] {
