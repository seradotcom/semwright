...
|---|---|---|
| R02 | Real owner-approved GNOME Wayland sessions now certify portal consent, keyboard+pointer `ConnectToEIS`, focused EIS pointer move/click against a disposable Wayland target, exact relative logical deltas, focus-drift rejection with no fallback, explicit stop and inactive post-stop state. GNOME exposes keycode-only `ei_keyboard` rather than `ei_text`; XKB-keymap translation is protocol-tested but still needs a live keyboard retest. Cancellation of an in-flight input operation and the broader portal-granted desktop matrix also remain uncertified. | Live-retest focused keyboard input through the EIS-advertised XKB keymap, cancel an in-flight input request cleanly, and extend portal-granted evidence to additional supported Wayland desktops where feasible. |
