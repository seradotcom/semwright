...
pub(super) fn operations() -> Vec<Op> {
    ...
    vec![
        ...
        op("dev.measurement.list", "List bounded measurements on the current Dev Mode page", ReadRisk, ReadOnly, true),
        op("dev.measurement.for_node", "List measurements connected to one node in Dev Mode", ReadRisk, ReadOnly, true),
        op("dev.measurement.add", "Add a Dev Mode measurement between two nodes", MutatingReversible, NonIdempotent, true),
        op("dev.measurement.edit", "Edit a Dev Mode measurement offset or label", MutatingReversible, Idempotent, true),
        op("dev.measurement.remove", "Delete a Dev Mode measurement", MutatingReversible, Idempotent, true),
        op("variable.collection.extend", "Create an Enterprise extended variable collection from a local collection", MutatingReversible, NonIdempotent, true),
        op("variable.values_for_collection", "Read inherited and overridden values for an extended collection", ReadRisk, ReadOnly, true),
        op("variable.override.remove_mode", "Remove one extended-mode variable override", MutatingReversible, Idempotent, true),
        op("variable.collection.overrides.remove_variable", "Remove all overrides for one variable from an extended collection", MutatingReversible, Idempotent, true),
        op("variable.resolve_for_consumer", "Resolve a variable for a concrete scene-node consumer", ReadRisk, ReadOnly, true),
        op("style.consumers.list", "List bounded nodes and fields consuming a style", ReadRisk, ReadOnly, true),
        op("textreview.status", "Inspect whether the Text Review plugin profile is enabled", ReadRisk, ReadOnly, true),
        op("textreview.enable", "Request user consent to enable the Text Review profile", MutatingReversible, NonIdempotent, false),
        op("textreview.disable", "Disable the Text Review profile", MutatingReversible, Idempotent, false),
    ]
}
