use super::{Op, op};
use semwright_types::{Idempotency, Risk};

pub(super) fn operations() -> Vec<Op> {
    use Idempotency::{Idempotent, NonIdempotent, ReadOnly};
    use Risk::{MutatingReversible, PrivilegeSensitive, ReadOnly as ReadRisk, SecretAccess};

    vec![
        op(
            "style.order.after",
            "Reorder a local style after another style of the same kind",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "style.folder.order.after",
            "Reorder a local style folder after a sibling folder",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "slides.grid.inspect",
            "Inspect the legacy Slides grid using the official Plugin API",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "slides.grid.set",
            "Replace the legacy Slides grid with an explicit complete node grid",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "annotation.category.inspect",
            "Inspect one annotation category by ID",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "font.load",
            "Load a font family/style for subsequent text mutation",
            ReadRisk,
            Idempotent,
            true,
        ),
        op(
            "dev.focused_node",
            "Inspect the focused node in Dev Mode, Slides, or Buzz",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "codegen.status",
            "Inspect Figma Dev Mode codegen preferences and mode",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "codegen.refresh",
            "Request a refresh of the active Dev Mode codegen output",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "payments.status",
            "Read the current plugin payment status",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "payments.first_run_age",
            "Read seconds since the current user first ran this plugin",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "payments.checkout",
            "Open the official Figma checkout flow for this plugin",
            PrivilegeSensitive,
            NonIdempotent,
            false,
        ),
        op(
            "payments.checkout.request",
            "Request deferred checkout from a restricted plugin mode",
            PrivilegeSensitive,
            NonIdempotent,
            false,
        ),
        op(
            "payments.dev.status.set",
            "Set the development-only payment status used by Figma",
            PrivilegeSensitive,
            Idempotent,
            false,
        ),
        op(
            "user.current",
            "Read the current Figma user when the currentuser permission is granted",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "figjam.active_users",
            "Read bounded active FigJam collaborators when activeusers permission is granted",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "node.top_level_frame",
            "Resolve the enclosing top-level Figma frame",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "node.plugin_data.keys",
            "List private plugin-data keys stored by this plugin",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "node.shared_plugin_data.get",
            "Read explicitly namespaced shared plugin data",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "node.shared_plugin_data.keys",
            "List explicitly namespaced shared plugin-data keys",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "node.shared_plugin_data.set",
            "Set or clear explicitly namespaced shared plugin data",
            PrivilegeSensitive,
            Idempotent,
            true,
        ),
        op(
            "library.publish_status.inspect",
            "Inspect a publishable component, component set, or style status",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "figjam.stamp.author.inspect",
            "Read the author metadata for a FigJam stamp",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "node.resize_unconstrained",
            "Resize a node without applying child constraints",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "node.aspect_ratio.lock",
            "Lock a node to its current aspect ratio",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "node.aspect_ratio.unlock",
            "Remove a node aspect-ratio lock",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "instance.overrides.remove_all",
            "Remove all direct instance overrides",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "layout.grid.rows.reorder",
            "Reorder explicit grid rows",
            MutatingReversible,
            NonIdempotent,
            true,
        ),
        op(
            "layout.grid.columns.reorder",
            "Reorder explicit grid columns",
            MutatingReversible,
            NonIdempotent,
            true,
        ),
        op(
            "layout.grid.child.position",
            "Move a direct grid child to an explicit cell",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "slides.transition.inspect",
            "Inspect a slide transition",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "slides.transition.set",
            "Set a complete slide transition",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "textreview.status",
            "Inspect whether the Semwright text-review integration is enabled",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "textreview.enable",
            "Request user approval to enable Semwright as a Figma text-review plugin",
            MutatingReversible,
            Idempotent,
            false,
        ),
        op(
            "textreview.disable",
            "Disable Semwright text-review integration when it is enabled",
            MutatingReversible,
            Idempotent,
            false,
        ),
        op(
            "widget.find_by_widget_id",
            "Find bounded widget nodes with a given widget ID",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "dev.measurement.list",
            "List bounded measurements on the current Dev Mode page",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "dev.measurement.for_node",
            "List measurements connected to one node in Dev Mode",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "dev.measurement.add",
            "Add a Dev Mode measurement between two nodes",
            MutatingReversible,
            NonIdempotent,
            true,
        ),
        op(
            "dev.measurement.edit",
            "Edit a Dev Mode measurement offset or label",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "dev.measurement.remove",
            "Delete a Dev Mode measurement",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "variable.collection.extend",
            "Create an Enterprise extended variable collection from a local collection",
            MutatingReversible,
            NonIdempotent,
            true,
        ),
        op(
            "variable.values_for_collection",
            "Read inherited and overridden values for an extended collection",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "variable.override.remove_mode",
            "Remove one extended-mode variable override",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "variable.collection.overrides.remove_variable",
            "Remove all overrides for one variable from an extended collection",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "variable.resolve_for_consumer",
            "Resolve a variable for a concrete scene-node consumer",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "style.consumers.list",
            "List bounded nodes and fields consuming a style",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "object.plugin_data.get",
            "Read private plugin data from an explicit semantic target",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "object.plugin_data.set",
            "Set or clear private plugin data on an explicit semantic target",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "object.plugin_data.keys",
            "List private plugin-data keys on an explicit semantic target",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "object.shared_plugin_data.get",
            "Read namespaced shared plugin data from an explicit semantic target",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "object.shared_plugin_data.set",
            "Set or clear namespaced shared plugin data on an explicit semantic target",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "object.shared_plugin_data.keys",
            "List namespaced shared plugin-data keys on an explicit semantic target",
            SecretAccess,
            ReadOnly,
            false,
        ),
        op(
            "style.variable.bind",
            "Bind or unbind a variable on a text-style field",
            MutatingReversible,
            Idempotent,
            true,
        ),
        op(
            "object.property.get",
            "Read an allowlisted public property from a style, variable, or collection",
            ReadRisk,
            ReadOnly,
            true,
        ),
        op(
            "object.property.set",
            "Set an allowlisted writable property on a local style, variable, or collection",
            MutatingReversible,
            Idempotent,
            true,
        ),
    ]
}
