...
10. Rust validates frame names/count, dimensions, PNG decode, pixel hash and alpha evidence before returning artifact paths. Large-sequence PNG decoding/hashing runs on Tokio's blocking pool so the current-thread Driver Protocol loop stays responsive while the artifact is certified.
