...
fn unique<'a>(ids: impl Iterator<Item = &'a str>, max: usize) -> Result<()> {
    ...
    for id in ids {
            ...
            security::identifier(id) && seen.insert(id) && seen.len() <= max,
            "Invalid, duplicate or excessive IDs",
        )?;
    }
    Ok(())
}
...
pub fn color(value: &str, theme: &Theme) -> Result<String> {
    let value = if let Some(key) = value.strip_prefix('@') {
        ...
        security::literal_color(value),
        "Color must be a literal color or theme token",
    )?;
    Ok(value.into())
}
pub fn settings(settings: &Settings) -> Result<()> {
    ensure(
        (16..=4096).contains(&settings.width)
            && (16..=4096).contains(&settings.height)
            && u64::from(settings.width) * u64::from(settings.height) <= 8_847_360,
    ...
    if let Some(bg) = &settings.background {
            ...
            security::literal_color(bg),
            "Background must be a literal color",
        )?;
    }
    Ok(())
}
pub fn theme(theme: &Theme) -> Result<()> {
        ...
        security::font(&theme.font_family) && security::font(&theme.mono_family),
        "Invalid font family",
    )?;
    ensure(
        number(theme.font_size, 4.0, 512.0)
            && (100..=900).contains(&theme.font_weight)
            && number(theme.spacing, 0.0, 1024.0)
            && number(theme.line_width, 0.0, 64.0)
            && number(theme.radius, 0.0, 1024.0),
        "Theme numeric bounds exceeded",
    )?;
    ensure(
        theme.colors.len() <= 32
            && theme.colors.contains_key("ink")
            && theme.colors.contains_key("surface"),
        "Theme requires ink/surface and at most 32 colors",
    )?;
    for (key, value) in &theme.colors {
        ensure(
            security::identifier(key) && security::literal_color(value),
            "Invalid theme color",
        )?;
    }
    Ok(())
...
pub fn properties(node: &Node, theme: &Theme) -> Result<()> {
    ...
    for (v, min, max) in [
        ...
        (p.start, 0.0, 1.0),
        (p.end, 0.0, 1.0),
        (p.arrow_size, 0.0, 256.0),
        (p.playback_rate, 0.1, 8.0),
        (p.zoom, 0.01, 100.0),
    ] {
        if let Some(v) = v {
            ensure(number(v, min, max), "Property numeric bounds exceeded")?;
        }
    }
    if let Some(v) = p.font_weight {
        ensure((100..=900).contains(&v), "Font weight exceeds bounds")?;
    }
    if let Some(v) = &p.font_family {
        ensure(security::font(v), "Invalid font family")?;
    }
    for v in [&p.fill, &p.stroke].into_iter().flatten() {
        color(v, theme)?;
    }
    if let Some(v) = &p.text {
        ensure(v.len() <= MAX_TEXT, "Text exceeds bounds")?;
    }
    if let Some(v) = &p.code {
        ensure(v.len() <= MAX_CODE, "Code text exceeds bounds")?;
    }
    if let Some(v) = &p.svg {
    ...
    if let Some(selection) = &p.selection {
        let lines = p
            ...
            .collect::<Vec<_>>();
        match selection {
            CodeSelection::Lines { start, end } => ensure(
                start <= end && (*end as usize) < lines.len(),
                "Code line selection is out of range",
            )?,
            CodeSelection::Word {
                line,
                start,
                ...
                let Some(text) = lines.get(*line as usize) else {
                    return Err(Error::invalid("Code line selection is out of range"));
                };
                ensure(
...
pub fn animation_times(scene: &Scene, animation: &Animation) -> Result<(u64, u64)> {
    let base = if let Some(id) = &animation.at.cue {
        scene
            ...
            .iter()
            .find(|c| &c.id == id)
            .ok_or_else(|| Error::invalid("Unknown animation cue"))?
            .time_ms
    } else {
        0
    };
    let start = i128::from(base) + i128::from(animation.at.offset_ms);
        ...
        start >= 0 && start <= i128::from(MAX_SCENE_MS),
        "Animation start exceeds bounds",
    )?;
    let duration = if let Some(id) = &animation.duration_cue {
        scene
            ...
            .iter()
            .find(|c| &c.id == id)
            .ok_or_else(|| Error::invalid("Unknown duration cue"))?
            .duration_ms
    } else {
    ...
    let end = u64::try_from(start)
        ...
        .ok_or_else(|| Error::invalid("Animation time overflow"))?;
...
fn animated_value(
    ...
    match (property, value) {
        (Position, AnimatedValue::Vector(v)) => ensure(
            ...
            "Animated position exceeds bounds",
        ),
        (Scale, AnimatedValue::Vector(v)) => ensure(
            v.iter().all(|v| number(*v, 0.001, 100.0)),
            "Animated scale exceeds bounds",
        ),
        (Fill | Stroke, AnimatedValue::Text(s)) => color(s, theme).map(|_| ()),
        (Text, AnimatedValue::Text(s)) => {
            ensure(s.len() <= MAX_TEXT, "Animated text exceeds bounds")
        }
        (Code, AnimatedValue::Text(s)) => {
            ensure(s.len() <= MAX_CODE, "Animated code exceeds bounds")
        }
        (Opacity | LineStart | LineEnd, AnimatedValue::Number(v)) => {
            ensure(number(*v, 0.0, 1.0), "Animated progress exceeds bounds")
        }
        (Width | Height | Radius, AnimatedValue::Number(v)) => {
            ensure(number(*v, 0.0, 16384.0), "Animated size exceeds bounds")
        }
        (FontSize, AnimatedValue::Number(v)) => {
            ensure(number(*v, 4.0, 512.0), "Animated font size exceeds bounds")
        }
        (LetterSpacing, AnimatedValue::Number(v)) => {
            ensure(number(*v, -100.0, 200.0), "Animated spacing exceeds bounds")
        }
        (CameraZoom, AnimatedValue::Number(v)) => ensure(
            number(*v, 0.01, 100.0),
            "Animated camera zoom exceeds bounds",
        ),
        (X | Y, AnimatedValue::Number(v)) => ensure(
            number(*v, -32768.0, 32768.0),
            "Animated position exceeds bounds",
        ),
        (Rotation, AnimatedValue::Number(v)) => ensure(
            number(*v, -36000.0, 36000.0),
            "Animated rotation exceeds bounds",
        ),
        (CameraFocus, AnimatedValue::Text(s)) => {
            ensure(security::identifier(s), "Invalid camera focus target")
        }
...
pub fn animations(scene: &Scene, theme: &Theme) -> Result<()> {
    ...
    for animation in &scene.animations {
        ...
        if animation.property == AnimatedProperty::CameraFocus {
                ...
                node.kind == NodeKind::Camera && animation.from.is_none(),
                "Camera focus requires a camera and has no from value",
            )?;
            if let AnimatedValue::Text(id) = &animation.to {
                ensure(
                    scene.nodes.iter().any(|n| &n.id == id),
                    "Unknown camera focus target",
                )?;
            }
        }
...
pub fn project_valid(project: &Project) -> Result<()> {
        ...
        security::identifier(&project.id)
            && project.generation.len() == 32
            && project.generation.bytes().all(|b| b.is_ascii_hexdigit())
            && project.revision > 0,
        "Invalid project identity",
    )?;
    ensure(
        serde_json::to_vec(project)?.len() <= MAX_PROJECT_BYTES,
        "Project exceeds byte limit",
    )?;
    ...
    unique(
        project
            .scenes
            .iter()
            .flat_map(|s| s.animations.iter().map(|a| a.id.as_str())),
        MAX_ANIMATIONS,
    )?;
    unique(
        project
            .scenes
            .iter()
            .flat_map(|s| s.cues.iter().map(|c| c.id.as_str())),
        MAX_CUES,
    )?;
    ...
    unique(project.audio.iter().map(|a| a.id.as_str()), 16)?;
        ...
        project
            .scenes
            .iter()
            .all(|s| (1..=MAX_SCENE_MS).contains(&s.duration_ms))
            && project.duration_ms() <= MAX_PROJECT_MS,
        "Project duration exceeds bounds",
    )?;
    for asset in &project.assets {
        security::relative_path(&asset.path)?;
        ensure(
            asset.path.starts_with("assets/")
                && security::digest(&asset.sha256)
                && asset.bytes > 0
                && asset.bytes <= MAX_ASSET_BYTES as u64,
            "Invalid asset metadata",
        )?;
        ensure(
            asset.provenance.as_ref().is_none_or(|s| s.len() <= 2048)
                && asset.license.as_ref().is_none_or(|s| s.len() <= 512),
            "Asset provenance exceeds bounds",
        )?;
        if let Some([w, h]) = asset.dimensions {
            ensure(
                w > 0
                    && h > 0
                    && w <= 4096
                    && h <= 4096
                    && u64::from(w) * u64::from(h) <= 8_847_360,
                "Asset dimensions exceed bounds",
            )?;
        }
        if let Some(ms) = asset.duration_ms {
            ensure(ms <= MAX_PROJECT_MS, "Asset duration exceeds bounds")?;
        }
    }
    for audio in &project.audio {
            ...
            project
                .assets
                .iter()
                .any(|a| a.id == audio.asset && a.kind == AssetKind::Audio)
                && number(audio.volume, 0.0, 1.0)
                && audio.offset_ms.unsigned_abs() <= MAX_PROJECT_MS,
            "Invalid audio track",
        )?;
    }
    for scene in &project.scenes {
        ...
        for node in &scene.nodes {
            ...
            while let Some(parent) = &current.parent {
                    ...
                    seen.insert(parent.as_str()) && seen.len() <= 64,
                    "Node hierarchy contains a cycle or exceeds depth",
                )?;
                current = map
                    .get(parent.as_str())
            ...
            if let Some(edge) = &node.properties.edge {
                    ...
                    node.properties.points.is_none(),
                    "A diagram edge cannot also define fixed points",
                )?;
                let from = map
                    .get(edge.from.as_str())
                    .ok_or_else(|| Error::invalid("Unknown edge start"))?;
                let to = map
                    .get(edge.to.as_str())
                    .ok_or_else(|| Error::invalid("Unknown edge end"))?;
                ensure(
                    from.parent == node.parent && to.parent == node.parent,
                    "Diagram endpoints must share the edge's parent coordinate space",
                )?;
            }
            if let Some(id) = &node.properties.asset {
                let asset = project
                    ...
                    .iter()
                    .find(|a| &a.id == id)
                    .ok_or_else(|| Error::invalid("Unknown asset reference"))?;
                let expected = match node.kind {
                    NodeKind::Image => AssetKind::Image,
                    NodeKind::Video => AssetKind::Video,
                    NodeKind::Svg => AssetKind::Svg,
                    _ => return Err(Error::invalid("Node does not support assets")),
                };
                ensure(asset.kind == expected, "Asset kind does not match node")?;
            }
            if matches!(node.kind, NodeKind::Image | NodeKind::Video) {
                ensure(
                    node.properties.asset.is_some(),
                    "Media node requires an asset",
                )?;
            }
            if node.kind == NodeKind::Svg {
                ensure(
                    node.properties.svg.is_some() ^ node.properties.asset.is_some(),
                    "SVG requires exactly one inline or local source",
                )?;
            }
        }
        ...
        for cue in &scene.cues {
                ...
                !cue.name.is_empty()
                    && cue.name.len() <= 128
                    && names.insert(&cue.name)
                    && cue.time_ms <= scene.duration_ms
                    && cue.duration_ms <= scene.duration_ms - cue.time_ms,
                "Invalid, duplicate or out-of-range cue",
            )?;
        }
...
pub fn ms_to_frames(ms: u64, fps: u32) -> Result<u64> {
    ...
    u64::try_from((u128::from(ms) * u128::from(fps) + 500) / 1000)
        .map_err(|_| Error::invalid("Frame conversion overflow"))
}
...
pub fn render_plan(project: &Project, profile: &RenderProfile) -> Result<RenderPlan> {
    ...
    let duration = ms_to_frames(project.duration_ms(), project.settings.fps)?;
        ...
        "Scaled render resolution exceeds bounds",
    )?;
    ensure(
        profile.first_frame < profile.end_frame_exclusive
            && profile.end_frame_exclusive <= duration
    ...
    Ok(RenderPlan {
        renderer: "bundled_browser_v1".into(),
        width,
        height,
        ...
        project_duration_ms: project.duration_ms(),
        alpha: profile.transparent || project.settings.background.is_none(),
