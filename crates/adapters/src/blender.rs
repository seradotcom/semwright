...
use semwright_protocol::{current_uid, read_frame, validate_peer, write_frame};
use semwright_types::*;
...
impl Blender {
    ...
    async fn connect(&self) -> Result<UnixStream> {
        let meta = std::fs::symlink_metadata(&self.socket).map_err(|_| {
            Error::unavailable("Blender add-on socket is absent; enable the add-on explicitly")
        })?;
            ...
            || meta.mode() & 0o777 != 0o600
        {
            return Err(Error::new(
                ...
                "Blender socket must be owned by this user and mode 0600",
            ));
        }
        ...
        if hello.get("type").and_then(Value::as_str) != Some("ready")
            || hello.get("protocol").and_then(Value::as_u64) != Some(1)
        {
            return Err(Error::new(
                ErrorCode::PluginProtocolError,
                "Blender protocol handshake failed",
            ));
        }
        Ok(stream)
    }
...
impl Backend for Blender {
    ...
    async fn execute(&self, ctx: &Context, command: &str, args: &Value) -> Result<Value> {
        ...
        write_frame(
            &mut stream,
            &json!({"type":"execute","id":id,"command":command,"args":args}),
        )
        .await?;
        ...
        if response.get("ok").and_then(Value::as_bool) == Some(true) {
            response
                ...
                .ok_or_else(|| {
                    Error::new(
                        ErrorCode::PluginProtocolError,
                        "Blender response must contain an object",
                    )
                    .uncertain()
                })
        } else {
            ...
            Err(Error::new(
                error,
                "Blender rejected or failed the typed action; host details are redacted",
            )
            .uncertain())
        }
    }
