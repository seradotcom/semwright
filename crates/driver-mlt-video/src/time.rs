//! Timeline/source intervals are [start, end), MLT in/out are inclusive. No float time math.
...
impl FrameRange {
    ...
    pub fn from_inclusive(start: u64, out: u64) -> Result<Self> {
        Self::new(
            start,
            out.checked_add(1)
                .ok_or_else(|| Error::invalid("MLT out overflow"))?,
        )
    }
...
impl FrameRate {
    ...
    /// MLT millisecond timestamps are quantized: nearest frame, ties up, not truncation.
    ...
    pub fn parse_mlt(self, text: &str) -> Result<u64> {
        if text.len() > 32 {
        ...
        if parts.len() != 3 {
            return Err(Error::invalid("Expected MLT frame or hh:mm:ss.mmm"));
        }
        ...
        if m >= 60 || frac.len() != 3 {
            return Err(Error::invalid("Invalid MLT clock"));
        }
