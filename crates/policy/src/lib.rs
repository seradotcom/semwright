...
impl Policy {
    pub fn new(config: PolicyConfig) -> Result<Self> {
        ...
        if config.profile != Profile::Observe {
            granted.extend(
                ["window.manage", "ui.invoke", "notifications.send"]
                    .into_iter()
                    .map(String::from),
            );
        }
        ...
        if granted.contains("shell.exec") {
            return Err(Error::invalid(
                "This build deliberately exposes no unrestricted shell",
            ));
        }
        ...
        for root in &config.filesystem {
            ...
            {
                return Err(Error::invalid(
                    "Filesystem root names must be unique identifiers",
                ));
            }
            if !root.path.is_absolute() || root.path == std::path::Path::new("/") {
                return Err(Error::invalid(
                    "Filesystem roots must be absolute and cannot be /",
                ));
            }
        }
    ...
    pub fn check(
        ...
        if !self.config.apps.is_empty() {
            if let Some(app) = target_app.or_else(|| args.get("app").and_then(Value::as_str)) {
                if !self.config.apps.contains(app) {
                    return Decision::Deny("Target application is outside the configured scope");
                }
            } else if command.risk.mutates() && command.name.starts_with("ui.") {
                return Decision::Deny(
                    "Scoped UI mutation requires a resolved application identity",
                );
            }
        }
    ...
    pub fn enforce(
        ...
        match self.check(command, args, target_app) {
            ...
            Decision::RequireConfirmation => Ok(true),
...
mod tests {
    ...
    fn descriptor(cap: &str, risk: Risk) -> CommandDescriptor {
        CommandDescriptor {
    ...
    #[test]
    fn observe_cannot_mutate() {
        let p = Policy::new(PolicyConfig::default()).unwrap();
        assert!(matches!(
            p.check(
                &descriptor("ui.invoke", Risk::Mutating),
                &serde_json::json!({}),
                None
            ),
            Decision::Deny(_)
    ...
    #[test]
    fn dangerous_cannot_self_approve() {
        let mut c = PolicyConfig::default();
        c.allow.insert("clipboard.read".into());
        let p = Policy::new(c).unwrap();
        assert_eq!(
            p.check(
                &descriptor("clipboard.read", Risk::SecretAccess),
    ...
    fn explicit_deny_wins() {
        let mut c = PolicyConfig::default();
        c.allow.insert("ui.invoke".into());
        c.deny.insert("ui.invoke".into());
        let p = Policy::new(c).unwrap();
        assert!(!p.capabilities().contains("ui.invoke"));
    }
    #[test]
    fn desktop_does_not_grant_clipboard_or_shell() {
        let c = PolicyConfig {
    ...
    fn terminal_controls_are_escaped() {
        let summary = confirmation_summary(
            "test",
            &serde_json::json!({"name":"\u{1b}[2J","text":"supersecret"}),
        );
