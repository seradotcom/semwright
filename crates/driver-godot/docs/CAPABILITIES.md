...
The Godot driver exposes **188 curated `driver.godot.*` capabilities**. Every advertised
...
Godot application refs bind node/resource/scene identity to provider, project, authenticated
session, generation, revision and fingerprint. Inspect operations return refs directly, while
`ref.resolve` refreshes identity and can fail closed on stale generation, missing targets,
class changes or stale revision/fingerprint. These refs remain provider-owned data; they are
not executable handles.
