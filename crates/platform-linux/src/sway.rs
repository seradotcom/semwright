...
impl Default for Sway {
    fn default() -> Self {
        Self {
            socket: std::env::var_os("SWAYSOCK")
...
impl Sway {
    ...
    async fn request(&self, kind: u32, payload: &str) -> Result<Value> {
        let path = self
            .socket
            .as_ref()
            .ok_or_else(|| Error::unavailable("SWAYSOCK/I3SOCK is not configured"))?;
        let mut stream = UnixStream::connect(path).await?;
        // SAFETY: getuid has no pointer/lifetime preconditions.
        if stream.peer_cred()?.uid() != unsafe { libc::getuid() } {
            return Err(Error::new(
                ErrorCode::PermissionDenied,
                "Compositor socket UID mismatch",
            ));
        }
        if payload.len() > 4096 {
            return Err(Error::invalid("Compositor request is too large"));
        }
        let mut header = b"i3-ipc".to_vec();
        ...
        let length = u32::from_le_bytes(
            header[6..10]
                .try_into()
                .map_err(|_| Error::invalid("Bad IPC header"))?,
        ) as usize;
        let response_kind = u32::from_le_bytes(
            header[10..14]
                .try_into()
                .map_err(|_| Error::invalid("Bad IPC type"))?,
        );
        if length > 4_194_304 || response_kind != kind {
            return Err(Error::new(
                ErrorCode::BackendFailed,
                "Sway response exceeds contract limits",
            ));
        }
    ...
    pub async fn windows(&self) -> Result<Vec<Value>> {
        let tree = self.request(4, "").await?;
        Ok(flatten_windows(&tree))
    }
    fn target(window: &Value) -> Result<NativeTarget> {
        let id = window["id"].as_u64().ok_or_else(|| {
            Error::new(
                ErrorCode::BackendFailed,
                "Sway returned an invalid window id",
            )
        })?;
        let app = window["app_id"]
            .as_str()
            .or_else(|| window["window_properties"]["class"].as_str())
            .unwrap_or("");
        Ok(NativeTarget {
            kind: "win".into(),
            identity: id.to_string(),
            revision: 0,
            fingerprint: format!("{}:{app}", window["pid"]),
            app: app.into(),
        })
    }
}
...
impl Backend for Sway {
    ...
    async fn probe(&self) -> Vec<Feature> {
        ...
        vec![feature(
            ...
            "window.manage",
            ready,
            "Native i3/Sway socket handshake",
            "Run inside Sway/i3 and preserve the owner-provided SWAYSOCK/I3SOCK",
        )]
    }
    async fn execute(&self, ctx: &Context, c: &str, args: &Value) -> Result<Value> {
        ...
        if c == "window.list" {
            ...
            for v in self.windows().await? {
                windows.push(json!({"ref":target_marker(Self::target(&v)?),"title":v["name"],"app":v["app_id"],"focused":v["focused"],"bounds":v["rect"],"coordinate_space":"compositor_logical"}));
            }
        ...
        let id = target
            .identity
            .parse::<u64>()
            .map_err(|_| Error::invalid("Invalid Sway target"))?;
        let instruction = match c {
            ...
            _ => {
                return Err(Error::new(
                    ...
                    "Unsupported Sway command",
                ));
            }
        };
        ctx.check_cancelled()?;
        let result = self
            .request(0, &format!("[con_id={id}] {instruction}"))
            .await
            .map_err(Error::uncertain)?;
        let replies = result.as_array().ok_or_else(|| {
            Error::new(ErrorCode::BackendFailed, "Malformed Sway command response").uncertain()
        })?;
        if replies.is_empty() || !replies.iter().all(|v| v["success"].as_bool() == Some(true)) {
            return Err(Error::new(
                ErrorCode::BackendFailed,
                "Compositor rejected operation; tiled geometry constraints may apply",
            )
            .uncertain());
        }
    ...
    async fn validate(&self, t: &NativeTarget) -> Result<()> {
        for w in self.windows().await? {
            let current = Self::target(&w)?;
            if current.identity == t.identity && current.fingerprint == t.fingerprint {
                return Ok(());
            }
        }
        Err(Error::new(
            ErrorCode::StaleReference,
            "Sway window identity changed",
        ))
    }
    async fn is_focused(&self, t: &NativeTarget) -> Result<bool> {
        for w in self.windows().await? {
            let current = Self::target(&w)?;
            if current.identity == t.identity && current.fingerprint == t.fingerprint {
                return Ok(w["focused"].as_bool().unwrap_or(false));
            }
        }
        Ok(false)
    }
...
mod tests {
    ...
    fn container_is_not_window() {
        assert!(flatten_windows(&json!({"type":"con","nodes":[]})).is_empty());
    }
    #[test]
    fn target_contains_no_command_source() {
        let t = Sway::target(&json!({"id":42,"pid":7,"app_id":"a"})).unwrap();
        assert_eq!(t.identity, "42");
    }
}
