...
5. A render job starts pinned Node only inside Driver Host. Node is capped with `--disable-wasm-trap-handler` and `--max-old-space-size=256`; the outer driver request remains at 4 GiB virtual address space and requests 256 tasks, the existing SDK hard maximum, because Firefox 151 WebRender returned `EAGAIN` while creating a thread under the previous 128-task request.
