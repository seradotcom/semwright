...
xcrun swiftc -swift-version 5 -parse-as-library -target "$arch-apple-macosx14.0" -emit-library -module-name SemwrightNative \
...
xcrun swiftc -swift-version 5 -parse-as-library "$repo/platforms/macos/tests/NativeSmoke.swift" \
