...
use semwright_platform_linux::hyprland::Hyprland;
...
#[ignore = "requires an isolated live Hyprland compositor and GTK fixture"]
async fn real_hyprland_window_lifecycle_uses_native_socket() {
    if std::env::var_os("SEMWRIGHT_TEST_HYPRLAND").is_none() {
        return;
    }
    assert_eq!(
        std::env::var("XDG_SESSION_TYPE").ok().as_deref(),
        Some("wayland"),
        "Hyprland live test must run on a Wayland compositor"
    );

    let runtime = std::env::var("XDG_RUNTIME_DIR").expect("XDG_RUNTIME_DIR required");
    let signature =
        std::env::var("HYPRLAND_INSTANCE_SIGNATURE").expect("Hyprland signature required");
    ...
    assert!(
        std::fs::metadata(&socket)
            .expect("Hyprland IPC socket metadata")
            .file_type()
            .is_socket()
    );

    let backend = Hyprland::default();
    let ctx = context();
    ...
    let mut child = tokio::process::Command::new("/usr/bin/zenity")
        ...
        .args([
            "--entry",
            "--title=Semwright Hyprland Fixture",
            "--text=Disposable Hyprland window",
        ])
        .spawn()
