...
impl SemanticDiff {
    pub fn is_empty(&self) -> bool {
        self.scenes_added.is_empty()
            && self.scenes_removed.is_empty()
            && self.nodes_added.is_empty()
            && self.nodes_removed.is_empty()
            && self.animations_changed.is_empty()
            && self.cues_changed.is_empty()
            && self.assets_changed.is_empty()
            && self.properties_changed.is_empty()
            && !self.scene_order_changed
            && self.node_order_changed.is_empty()
...
fn map<'a, T: Serialize + 'a>(
    ...
    values
        .map(|(id, value)| Ok((id.to_owned(), serde_json::to_value(value)?)))
        .collect()
}
...
pub fn between(before: &Project, after: &Project) -> Result<SemanticDiff> {
    ...
    out.scenes_added = b
        ...
        .cloned()
        .collect();
    out.scenes_removed = a
        .keys()
        .filter(|id| !b.contains_key(*id))
        .cloned()
        .collect();
    out.scene_order_changed = before.scenes.iter().map(|s| &s.id).collect::<Vec<_>>()
        != after.scenes.iter().map(|s| &s.id).collect::<Vec<_>>();
    let an = map(before
        .scenes
        .iter()
        .flat_map(|s| s.nodes.iter().map(|n| (n.id.as_str(), n))))?;
    let bn = map(after
        .scenes
        .iter()
        .flat_map(|s| s.nodes.iter().map(|n| (n.id.as_str(), n))))?;
    out.nodes_added = bn
        .keys()
        .filter(|id| !an.contains_key(*id))
        .cloned()
        .collect();
    out.nodes_removed = an
        .keys()
        .filter(|id| !bn.contains_key(*id))
        .cloned()
        .collect();
    for id in an.keys().filter(|id| bn.contains_key(*id)) {
        fields(
            ...
            &an[id],
            &bn[id],
            &mut out.properties_changed,
        );
    }
    for scene in &before.scenes {
        if let Some(other) = after.scenes.iter().find(|s| s.id == scene.id) {
            if scene.nodes.iter().map(|n| &n.id).collect::<Vec<_>>()
                != other.nodes.iter().map(|n| &n.id).collect::<Vec<_>>()
            {
    ...
    out.animations_changed = changed(
        &map(before
            .scenes
            .iter()
            .flat_map(|s| s.animations.iter().map(|a| (a.id.as_str(), a))))?,
        &map(after
            .scenes
            .iter()
            .flat_map(|s| s.animations.iter().map(|a| (a.id.as_str(), a))))?,
    ...
    out.cues_changed = changed(
        &map(before
            .scenes
            .iter()
            .flat_map(|s| s.cues.iter().map(|c| (c.id.as_str(), c))))?,
        &map(after
            .scenes
            .iter()
            .flat_map(|s| s.cues.iter().map(|c| (c.id.as_str(), c))))?,
