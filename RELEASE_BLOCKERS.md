...
|---|---|---|
| R02 | Real owner-approved GNOME Wayland sessions certify portal consent, keyboard+pointer `ConnectToEIS`, focused EIS pointer move/click against a disposable Wayland target, exact relative logical deltas, focus-drift rejection with no fallback, explicit stop and inactive post-stop state. GNOME exposes keycode-only `ei_keyboard`; XKB translation and cooperative in-flight text cancellation are now protocol-tested. **Live keyboard/cancellation certification remains open:** later keyboard runs on the owner's login were invalidated after injected text reached an unintended host application despite fixture focus checks, and a nested GNOME/private-D-Bus attempt was also not isolated from the host logind session. | Re-run focused XKB keyboard input and in-flight cancellation only inside a VM or genuinely independent seat/login that cannot inject into the owner's desktop, then extend portal-granted evidence to additional supported Wayland desktops where feasible. |
