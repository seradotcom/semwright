...
|---|---|---|
| R02 | Real owner-approved GNOME Wayland sessions certify portal consent, keyboard+pointer `ConnectToEIS`, focused EIS pointer move/click against a disposable Wayland target, exact relative logical deltas, focus-drift rejection with no fallback, explicit stop and inactive post-stop state. GNOME exposes keycode-only `ei_keyboard` rather than `ei_text`; XKB-keymap translation, cooperative in-flight cancellation, sender backpressure handling, pacing and bidirectional modifier-feedback handling have deterministic protocol coverage. Real keyboard target-delivery evidence is **not certified**: direct-desktop and nested-GNOME methods were invalidated after EIS input reached the owner-active ChatGPT text box despite semantic window-focus checks. Nested GNOME with separate HOME/runtime/D-Bus/Wayland was insufficient because RemoteDesktop/EIS remained associated with the real graphical login. | Re-run keyboard target delivery and cancellation only inside a genuinely separate graphical authority boundary (VM or independent seat/session), then extend portal-granted evidence to additional supported Wayland desktops. Do not use the owner-active login or nested-shell-on-the-same-login as keyboard certification environments. |
| R06 | Cross-desktop live evidence now includes GNOME Wayland, Plasma/KWin Wayland, headless Sway, Openbox/EWMH X11 and a hardware-backed nested Hyprland 0.56.2 run on a real AMD render node. The Hyprland run certifies native socket discovery/focus/move/resize, stale-reference rejection, cleanup and a synthetic second output at scale 1.25. The matrix is still incomplete because KWin-parent nested evidence is not a physical Hyprland login and broader real-login/restart/focus-drift/physical mixed-scale coverage remains. | Execute the remaining physical/real-login matrix and negative cases: compositor restart/reconnect, focus drift and cancellation where applicable, physical mixed-scale/multi-monitor where available, and cleanup. |
