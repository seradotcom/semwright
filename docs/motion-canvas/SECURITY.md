...
Ubuntu 24.04 additionally restricts unprivileged user namespaces through AppArmor. CI preserves that restriction and adapts only the distro bwrap profile's older first-exec rule to current upstream `pix` stacked-fallback semantics; it does not disable the system-wide control. CI loads the distro `bwrap-userns-restrict` profile specifically for `/usr/bin/bwrap`; it does not disable `kernel.apparmor_restrict_unprivileged_userns` system-wide. This lets Bubblewrap create the isolated namespaces it needs while preserving Ubuntu's global user-namespace mitigation for unrelated processes.
