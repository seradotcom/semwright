...
static func describe(ctx, args: Dictionary) -> Dictionary:
    var target_class := str(args.get("class", ""))
    ...
    var data := {
        "engine_version": str(Engine.get_version_info().get("string", "unknown")),
        "class": target_class,
        "parent": str(ClassDB.get_parent_class(target_class)),
        "api_type": _api_type_name(ClassDB.class_get_api_type(target_class)),
    ...
    if bool(args.get("properties", true)):
        data["properties"] = _class_properties(ctx, target_class, no_inheritance)
    if bool(args.get("methods", true)):
        data["methods"] = _methods(ctx, ClassDB.class_get_method_list(target_class, no_inheritance), MAX_METHODS)
    if bool(args.get("signals", true)):
        data["signals"] = _methods(ctx, ClassDB.class_get_signal_list(target_class, no_inheritance), MAX_SIGNALS)
    if bool(args.get("enums", true)):
        data["enums"] = _class_enums(target_class, no_inheritance)
...
static func _class_properties(ctx, target_class: String, no_inheritance: bool) -> Array:
    var rows := _properties_from_metadata(
        ctx,
        ClassDB.class_get_property_list(target_class, no_inheritance),
        MAX_PROPERTIES,
    )
    for row in rows:
        ...
        if not name.is_empty():
            ...
            row["getter"] = str(ClassDB.class_get_property_getter(target_class, name))
            row["setter"] = str(ClassDB.class_get_property_setter(target_class, name))
    return rows
...
static func _class_enums(target_class: String, no_inheritance: bool) -> Array:
    var rows: Array = []
    for raw_enum in ClassDB.class_get_enum_list(target_class, no_inheritance):
        if rows.size() >= MAX_ENUMS:
            break
        var enum_name := str(raw_enum)
        var constants: Array = []
        for raw_constant in ClassDB.class_get_enum_constants(target_class, enum_name, no_inheritance):
            if constants.size() >= MAX_CONSTANTS:
                break
            var constant := str(raw_constant)
            constants.append({
                "name": constant,
                "value": ClassDB.class_get_integer_constant(target_class, constant),
            })
        rows.append({
            "name": enum_name,
            "bitfield": ClassDB.is_class_enum_bitfield(target_class, enum_name, no_inheritance),
            "constants": constants,
        })
    return rows
